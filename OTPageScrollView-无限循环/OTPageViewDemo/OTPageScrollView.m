...
- (void)reloadData
{
    ...
    //*5表示左右各2两遍
    for (int i = 0; i < _numberOfCell*5; i ++) {
        UIView * cell = [self.dataSource pageScrollView:self viewForRowAtIndex:i%_numberOfCell];
    ...
    //*5表示左右各2两遍
    float scrollViewSizeWith = (startX - self.padding + (self.frame.size.width - _cellSize.width)/2)*5;
    self.contentSize = CGSizeMake(scrollViewSizeWith, 1);
    [self setContentOffset:CGPointMake(_numberOfCell*2*self.frame.size.width, 0)];
}
...
- (void)handleTapGesture:(UITapGestureRecognizer*)tapGesture
{
    ...
    if (yInCell && xInCell) {
        ...
        [self.delegate pageScrollView:self didTapPageAtIndex:xInCellNumber%_numberOfCell];
        //        [UIView animateWithDuration:0.3 animations:^{
        //            [self setContentOffset:CGPointMake((_cellSize.width + self.padding) * xInCellNumber, 0)];
        //        }];
        //bug：点击动画过渡时稍微有一些循环现象看出，无法修改
        [UIView animateWithDuration:0.3 animations:^{
            [self setContentOffset:CGPointMake((_numberOfCell*2+(xInCellNumber%_numberOfCell))*self.frame.size.width, 0)];
        }];
    }
}
